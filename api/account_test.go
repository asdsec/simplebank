...
import (
	...
	"github.com/lib/pq"
	mockdb "github.com/sametdmr/simplebank/db/mock"
	db "github.com/sametdmr/simplebank/db/sqlc"
	"github.com/sametdmr/simplebank/token"
	"github.com/sametdmr/simplebank/utility"
	"github.com/stretchr/testify/require"
)

var pqErrForeignKeyValidation = *&pq.Error{
	Code: "23503",
}

var pqErrUniqueValidation = *&pq.Error{
	Code: "23505",
}
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					CreateAccount(gomock.Any(), gomock.Any()).
					Times(1).
					Return(db.Account{}, &pqErrForeignKeyValidation)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					CreateAccount(gomock.Any(), gomock.Any()).
					Times(1).
					Return(db.Account{}, &pqErrUniqueValidation)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusForbidden, recorder.Code)
			},
